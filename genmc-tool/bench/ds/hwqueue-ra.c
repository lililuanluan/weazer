...
void enqueue(int a)
{
	...
	int k = atomic_fetch_add_explicit(&back, 1, memory_order_acquire);
...
const int n_noise = NOISE;

int main()
{
	pthread_t t1, t2, t3, t4;
	pthread_t noise[n_noise];
	...
	for(int i=0;i<n_noise;i++) {
		pthread_create(&noise[i], NULL, thread_noise, NULL);
	}
