...
static llvm::cl::opt<bool> clFuzz("fuzz", llvm::cl::cat(clGeneral),
	llvm::cl::desc("Run under fuzzing mode"));
...
static llvm::cl::opt<unsigned int> clEstimationMin(
	...
	llvm::cl::desc("Number of minimum allotted rounds for state-space estimation"));
...
static llvm::cl::opt<unsigned int> clFuzzMax(
	"fuzz-max", llvm::cl::init(1000), llvm::cl::value_desc("N"),
	llvm::cl::cat(clDebugging),
	llvm::cl::desc("Number of maximum allotted rounds for fuzzing"));

static llvm::cl::opt<MutationPolicy> clMutationPolicy(
	"mutation-policy", llvm::cl::cat(clDebugging), llvm::cl::value_desc("value"), llvm::cl::init(MutationPolicy::RevisitCut),
	llvm::cl::desc("Choose the mutation policy for fuzzing:"),
	llvm::cl::values(
		clEnumValN(MutationPolicy::NoMutation, "no-mutation", "randomly exploring graphs independently each time"),
		clEnumValN(MutationPolicy::RevisitCut, "revisit", "revisit cut (default)")
	)
);


static llvm::cl::opt<std::string> clDumpFuzzingCoveragePlotFile(
	"dump-coverage", llvm::cl::cat(clDebugging),
	llvm::cl::init(""), llvm::cl::value_desc("file"),
	llvm::cl::desc("Dump the coverage plot for fuzzing to a file (CSV format)")
);

static llvm::cl::opt<std::string> clDumpFuzzingValSeqsFile(
	"dump-value-sequence", llvm::cl::cat(clDebugging),
	llvm::cl::init(""), llvm::cl::value_desc("file"),
	llvm::cl::desc("Dump the seen value sequences for fuzzing to a file (DOT format)")
);

static llvm::cl::opt<bool>
clInteractive("interactive", llvm::cl::cat(clDebugging),
	llvm::cl::desc("interactively pick threads and rfs"));
...
static llvm::cl::opt<bool> clCountDistinctGraphNums("count-distinct-execs", llvm::cl::cat(clDebugging), llvm::cl::desc("Counts the number of distinct graphs (N_d), prints N_d / N"));

static llvm::cl::opt<bool>
	clFuzzValueNoblock("fuzz-value-noblock", llvm::cl::cat(clDebugging), llvm::cl::init(true),
			   llvm::cl::desc("Fuzzing: don't mutate rfs that will lead to blocking"));

static llvm::cl::opt<FuzzIsInteresting> clFuzzIsInteresting(
	"is-interesting", llvm::cl::cat(clDebugging), llvm::cl::value_desc("value"),
	llvm::cl::init(FuzzIsInteresting::NewGraphs),
	llvm::cl::desc("Desides whether a graph is interesting to be mutated"),
	llvm::cl::values(
		clEnumValN(FuzzIsInteresting::Always, "always", "always mutate graphs"),
		clEnumValN(FuzzIsInteresting::NewGraphs, "new", "mutate new graphs"),
		clEnumValN(FuzzIsInteresting::GraphFreq, "freq",
			   "mutate graphs with frequency lower than a threshold"),
		clEnumValN(FuzzIsInteresting::GraphFreqSqured, "freq-squared",
			   "mutate graphs with a squared frequency lower than a threshold"),
		clEnumValN(FuzzIsInteresting::Random, "rand",
			   "random number 0-1, returns true if lower than threshold"),
		clEnumValN(FuzzIsInteresting::ValueSeq, "val-seq",
			   "mutate graphs with rare value sequences")));

static llvm::cl::opt<float> clInterestingThreshold("interesting-threshold", llvm::cl::cat(clDebugging), llvm::cl::value_desc("float"), llvm::cl::init(1.), llvm::cl::desc("Fuzzing: a graph is interesting if its 'score' is lower than some threshold, a dimensionless quantity"));

static llvm::cl::opt<int> clNumMutationBound(
	"num-mutation", llvm::cl::cat(clDebugging), llvm::cl::value_desc("uint"),
	llvm::cl::init(-1),
	llvm::cl::desc("Fuzzing: the upper bound of the number of mutations on one graph"));

static llvm::cl::opt<bool> clTestHashCollision("test-collision", llvm::cl::cat(clDebugging), llvm::cl::desc("Hash: Testing the hash function"));

static llvm::cl::opt<bool>
	clUseExecQueue("use-queue", llvm::cl::cat(clDebugging), llvm::cl::init(true),
		       llvm::cl::desc("execStack: use queue or stack? default is stack"));

static llvm::cl::opt<bool> clPrioBackRev("prio-back-rev", llvm::cl::cat(clDebugging),
					 llvm::cl::init(false),
					 llvm::cl::desc("Fuzzing: prioritize backward revisits"));

static llvm::cl::opt<bool> clPrioStaleStore("prio-stale-stores", llvm::cl::cat(clDebugging),
					 llvm::cl::init(false),
					 llvm::cl::desc("Fuzzing: prioritize stale stores"));

static llvm::cl::opt<bool> clPrioNewVal("prio-new-val", llvm::cl::cat(clDebugging),
					 llvm::cl::init(false),
					 llvm::cl::desc("Fuzzing: prioritize new values"));

static llvm::cl::opt<size_t> clFuzzRestartRandomPeriod("insert-rand", llvm::cl::cat(clDebugging), llvm::cl::init(std::numeric_limits<size_t>::max()), llvm::cl::value_desc("uint"), llvm::cl::desc("Fuzzing: periodically insert random seeds"));

static llvm::cl::opt<unsigned int>
	clFuzzCorpusSize("fuzz-corpus", llvm::cl::cat(clDebugging), llvm::cl::init(0),
			 llvm::cl::value_desc("uint"),
			 llvm::cl::desc("Fuzzing: set the size of corpus of graphs for mutations"));

static llvm::cl::opt<bool> clFuzzFilterSeenVals(
	"filter-seen-vals", llvm::cl::cat(clDebugging), llvm::cl::init(false),
	llvm::cl::desc("Fuzzing: filter out mutations that would lead to reading seen values"));

static llvm::cl::opt<std::string> clDumpTimePerGraph(
	"dump-time", llvm::cl::cat(clDebugging), llvm::cl::init(""), llvm::cl::value_desc("file"),
	llvm::cl::desc("Dump the seconds taken per graph to a file (CSV format)"));

static llvm::cl::opt<AddMaxCoRf> clFuzzAddMaxCoRf(
	"add-max", llvm::cl::cat(clDebugging), llvm::cl::init(AddMaxCoRf::Never),
	llvm::cl::desc("Fuzzing: when to add co rf maximally during graph construction"),
	llvm::cl::values(clEnumValN(AddMaxCoRf::Never, "never", "never do this, always random"),
			 clEnumValN(AddMaxCoRf::Empty, "empty",
				    "only when starting from empty graphs"),
			 clEnumValN(AddMaxCoRf::Mutated, "mutated",
				    "only when starting from mutated (non-empty) graphs")));
...
void Config::checkConfigOptions() const
{
	...
	/* Checking fuzzing related options */
	if (clFuzzMax.getNumOccurrences() ||
		clDumpFuzzingCoveragePlotFile.getNumOccurrences() ||
		clDumpFuzzingValSeqsFile.getNumOccurrences() ||
		clFuzzIsInteresting.getNumOccurrences() ||
		clInterestingThreshold.getNumOccurrences() ||
		clFuzzRestartRandomPeriod.getNumOccurrences() ||
		clFuzzCorpusSize.getNumOccurrences() ||
		clFuzzFilterSeenVals.getNumOccurrences()) {
		if (clFuzz.getNumOccurrences() <= 0) {
			WARN("Fuzzing options set but not in fuzz mode!\n");
		}
	}
	if (clDumpFuzzingCoveragePlotFile.getNumOccurrences()) {
		const auto& f = clDumpFuzzingCoveragePlotFile.getValue();
		if (llvm::sys::fs::exists(f)) {
			WARN(f + " already exists, will be overwritten\n");
		}
		else {
			int fd;
			auto ec = llvm::sys::fs::openFileForWrite(f, fd, llvm::sys::fs::CD_OpenAlways);
			if (fd == -1) {
				ERROR(f + " is not a valid file name!");
			}
		}
	}

	if (clInterestingThreshold.getNumOccurrences() && clInterestingThreshold.getValue() < 0) {
		ERROR("threshold must be > 0!");
	}
	if (clNumMutationBound.getNumOccurrences() && clNumMutationBound.getValue() < 0) {
		ERROR("mutation bound must be > 0!");
	}
}
...
void Config::saveConfigOptions()
{
	...
	vLevel = clVLevel;
	dumpFuzzingCoveragePlotFile = clDumpFuzzingCoveragePlotFile;
	dumpFuzzingValSeqsFile = clDumpFuzzingValSeqsFile;
	dumpTimePerGraph = clDumpTimePerGraph;
#ifdef ENABLE_GENMC_DEBUG
...
#endif
	countDistinctExecs = clCountDistinctGraphNums;
	fuzzValueNoblock = clFuzzValueNoblock;
	fuzzIsInteresting = clFuzzIsInteresting;
	interestingThreshold = clInterestingThreshold;
	mutationBound = clNumMutationBound >= 0 ? std::optional(clNumMutationBound.getValue())
						: std::nullopt;
	testHashCollision = clTestHashCollision;
	useQueue = clUseExecQueue;
	fuzzRestartRandomPeriod = clFuzzRestartRandomPeriod;
	fuzzCorpusSize = clFuzzCorpusSize;
	fuzzFilterSeenVals = clFuzzFilterSeenVals;
	prioBackRev = clPrioBackRev;
	prioStaleStore = clPrioStaleStore;
	prioNewVal = clPrioNewVal;
	fuzzAddMaxCoRf = clFuzzAddMaxCoRf;
	interactiveAddGraph = clInteractive;
