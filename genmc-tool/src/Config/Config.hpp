...
enum class MutationPolicy { NoMutation, RevisitCut };
enum class FuzzIsInteresting { Always, NewGraphs, GraphFreq, GraphFreqSqured, Random, ValueSeq };
enum class AddMaxCoRf { Never, Empty, Mutated };
...
#endif
	bool countDistinctExecs;
	bool fuzzValueNoblock;
	std::string dumpFuzzingCoveragePlotFile;
	std::string dumpFuzzingValSeqsFile;
	FuzzIsInteresting fuzzIsInteresting;
	std::optional<unsigned int> mutationBound;
	float interestingThreshold;
	bool testHashCollision;
	bool useQueue = false;
	size_t fuzzRestartRandomPeriod;
	unsigned int fuzzCorpusSize;
	bool fuzzFilterSeenVals;
	std::string dumpTimePerGraph;
	AddMaxCoRf fuzzAddMaxCoRf;
	bool prioBackRev;
	bool prioStaleStore;
	bool prioNewVal;
	bool interactiveAddGraph;
