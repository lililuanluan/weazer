...
public:
	...
	bool addSeq(const Rng &s, Payload &&data)
	...
	template <typename Rng>
	const Payload *lookup(const Rng &s) const
	requires std::ranges::range<Rng> &&
		 std::same_as<std::ranges::range_value_t<Rng>, value_type>
	{
		Node *cNode = getRoot();
		Node *tNode = nullptr;
		auto sBeg = std::ranges::begin(s);
		auto sEnd = std::ranges::end(s);
	...
	const Payload *lookup(const Seq &s) const { return this->lookup<Seq>(s); }
	...
	void dumpDot(std::string gName, auto &&os)
	{
		os << "digraph " << gName << "{\n";
		auto declNode = [&](auto &&node) {
			os << "\tnode_" << node << "[label=\"" << format(node->label());
			if constexpr (requires { os << node->data(); }) {
				os << ": " << node->data();
			}
			os << "\"]\n";
		};
		auto pointTo = [&](auto &n1, auto &n2) {
			os << "\tnode_" << n1 << " -> "
			   << "node_" << n2 << '\n';
		};
		for (auto &n : nodes | std::views::drop(1)) {
			auto cur = n.get();
			declNode(cur);
