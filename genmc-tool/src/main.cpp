			      ...
			      const GenMCDriver::Result &res)
{
	...
	if (conf->countDistinctExecs) {
		auto total = res.totalExplored();
		auto distinct = res.totalExploredDistinct();
		PRINT(VerbosityLevel::Error)
			<< "\nNumber of distinct graphs: " << distinct << " (/" << total << " = "
			<< llvm::format("%.2f", (float)distinct / total * 100) << "%)";
		PRINT(VerbosityLevel::Error)
			<< "\nNumber of distinct complete executions explored: "
			<< res.graphFreqComplete.size();
		PRINT(VerbosityLevel::Error) << "\nNumber of distinct blocked executions seen: "
					     << res.graphFreqBlock.size();
	}
	if (conf->testHashCollision) {
		int cnt = std::count_if(res.hashCount.begin(), res.hashCount.end(),
					[](const auto &pair) { return pair.second.size() > 1; });

		if (cnt > 0)
			PRINT(VerbosityLevel::Error)
				<< "\nCollision: " << cnt << " / " << (res.totalExplored());
		else
			PRINT(VerbosityLevel::Error) << "\nNo repeated hash.";
	}
	if (conf->fuzz) {
		auto &stats = res.fuzzStats;
		PRINT(VerbosityLevel::Error)
			<< '\n'
			<< "random exploration: " << stats.randCount << '\n'
			<< "interesting graphs: " << stats.interestingCount << '\n'
			<< "mutation count: " << stats.mutationCount << '\n'
			<< "total work: " << stats.mutationCount + stats.randCount;
	}
