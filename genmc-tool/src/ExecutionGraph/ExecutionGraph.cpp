...
bool isPrefixOf(const ExecutionGraph &g1, const ExecutionGraph &g2) {
	if (g2.getNumThreads() < g1.getNumThreads())
		return false;
	for (auto i = 0U; i < g1.getNumThreads(); i++) {
		// fixme: hoist
		if (g2.getThreadSize(i) < g1.getThreadSize(i))
			return false;
		for (auto j = 0U; j < g1.getThreadSize(i); j++) {
			auto *lab1 = g1.getEventLabel(Event(i, j));
			auto *lab2 = g2.getEventLabel(Event(i, j));
			if (lab1->getKind() != lab2->getKind())
				return false;
			if (auto *rLab1 = llvm::dyn_cast<ReadLabel>(lab1)) {
				auto *rLab2 = llvm::dyn_cast<ReadLabel>(lab2);
				if ((rLab1->getRf() && !rLab2->getRf()) ||
					(!rLab2->getRf() && !rLab1->getRf()) ||
					rLab1->getRf()->getPos() !=
						rLab2->getRf()->getPos())
					return false;
			}
			if (auto *wLab1 = llvm::dyn_cast<WriteLabel>(lab1)) {
				auto *wLab2 = llvm::dyn_cast<WriteLabel>(lab2);
				if ((g1.co_imm_pred(wLab1) &&
						!g2.co_imm_pred(wLab2)) ||
					(!g2.co_imm_pred(wLab2) &&
						g1.co_imm_pred(wLab1)) ||
					(g1.co_imm_pred(wLab1) &&
						g2.co_imm_pred(wLab2) &&
						g1.co_imm_pred(wLab1)->getPos() !=
							g2.co_imm_pred(wLab2)->getPos()))
					return false;
			}
		}
	}
