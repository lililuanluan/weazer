...
protected:
	...
	using ValueBoolT =
		std::unordered_map<unsigned int, Trie<std::vector<SVal>, bool, SValUCmp>>;
	using ValueFreqT =
		std::unordered_map<unsigned int, Trie<std::vector<SVal>, unsigned int, SValUCmp>>;
	using ExecCacheT = std::unordered_set<std::unique_ptr<ExecutionGraph>>;
	using ChoiceMap = std::map<Stamp, VSet<std::pair<Event, double>>>;
	using GraphHashT = decltype(std::hash<ExecutionGraph>{}(ExecutionGraph{}));
	using GraphHashMap = std::unordered_map<GraphHashT, std::vector<std::string>>;
...
public:
	...
	using Mode = std::variant<VerificationMode, FuzzingMode, EstimationMode>;

	/* Fuzzing statistics */
	struct FuzzStats {

		std::vector<unsigned int> coverage; /* coverage plot. coverage[i] = num of distinct
						       graphs covered after i-th iteration */
		std::vector<float> secelapsed;
		double freqSum = 0; /* sum of the frequencies of graphs */

		/* count the number of interesting graphs, random explorations (rerun), etc */
		unsigned int randCount = 1;
	...
	struct Result {
...
#endif
		std::unordered_map<GraphHashT, unsigned int> graphFreqComplete, graphFreqBlock;
		/* maps hashes to execution graphs, used for testing hash collision */
		GraphHashMap hashCount;
		...
		auto totalExplored() const { return explored + exploredBlocked; }
		auto totalExploredDistinct() const
		{
			return graphFreqComplete.size() + graphFreqBlock.size();
		}
		size_t getGraphFreq(GraphHashT ghash) const;
		FuzzStats fuzzStats{};
	};

	void updateFuzzingStats(const ExecutionGraph &g);
	/* update the hash value of the latest graph if needed */
	void updateGraphFrequencies(const ExecutionGraph &g);

	/* update the weights of rf choices for fuzzing */
	void updateStSpaceChoicesWeights(const ExecutionGraph &g, ChoiceMap &choices);

	GraphHashT getLastHash()
	{
		if (!lastHash)
			lastHash = std::hash<ExecutionGraph>{}(getGraph());
		return *lastHash;
	}
	void clearLastHash() { lastHash = std::nullopt; }
	void recordAndDumpHashCollisions();
...
protected:
	...
	bool inFuzzingMode() const { return std::holds_alternative<FuzzingMode>(mode); }
	...
	/* Fuzzing: Returns true if the fuzzer runs enough iterations */
	bool shouldStopFuzzing() const
	{
		BUG_ON(!inFuzzingMode());
		return getResult().totalExplored() >= getConf()->fuzzMax;
	}

	/* Fuzzing: Returns true if the next iteration should be a random exploration, skipping the
	 * mutation part */
	bool shouldSkipFuzzing() const
	{
		auto total = getResult().totalExplored();
		return ((total % getConf()->fuzzRestartRandomPeriod) == 0) ||
		       (getResult().totalExplored() < getConf()->fuzzCorpusSize);
	}

	/* Fuzzing: compute a score for the current execution graph based on seen values*/
	float computeValSeqScore(const ExecutionGraph &g);
	float computeValSeqScore(const ExecutionGraph &g, int tid);

	/* Fuzzing: update seen values from a graph */
	void updateSeenValues(const ExecutionGraph &g);

	void cacheCurrentExecution();
...
private:
	/* Dumps the coverage plots, value sequences, etc */
	struct DumpGuard {

		std::ofstream fs;
		explicit DumpGuard(GenMCDriver *driver)
			: driver{driver},
			  dumpCoverage{!driver->getConf()->dumpFuzzingCoveragePlotFile.empty()},
			  dumpVal{!driver->getConf()->dumpFuzzingValSeqsFile.empty()},
			  dumpTime{!driver->getConf()->dumpTimePerGraph.empty()},
			  c_start{std::clock()}
		{
			if (!dumpCoverage)
				return;
			fs = std::ofstream{driver->getConf()->dumpFuzzingCoveragePlotFile};
			fs << "Iter,Cover,SecElapsed\n";
		}

		void tick() 
		{
			auto now = std::clock();
			float elapsed = float(now - c_start) / CLOCKS_PER_SEC;
			// llvm::dbgs() << "Iter: " << driver->getResult().totalExplored()
			// 	     << ", GraphSize: " << driver->getGraph().getMaxStamp()
			//  << ", Time: " << static_cast<size_t>(elapsed*1000) << "ms" << '\n';
			((GenMCDriver *)driver)
				->getResult()
				.fuzzStats.secelapsed.push_back(elapsed);

			const auto &stats = driver->getResult().fuzzStats;
			int i = stats.coverage.size() - 1;
			if (i > 0)
				fs << (i + 1) << "," << stats.coverage[i] << ","
				   << stats.secelapsed[i] << '\n';
		}
	...
	private:
		void dumpCoveragePlot() const
		{
			if (!dumpCoverage)
				return;
			// auto &f = driver->getConf()->dumpFuzzingCoveragePlotFile;
			// std::ofstream fs(f);
			// fs << "Iter,Cover,SecElapsed\n";
			// const auto &stats = driver->getResult().fuzzStats;
			// for (size_t i = 0; i < stats.coverage.size(); i++) {
			// 	fs << i + 1 << "," << stats.coverage[i] << ","
			// 	   << stats.secelapsed[i] << '\n';
			// }
		}
		...
		void dumpValueSeq()
		{
			if (!dumpVal || !driver->inFuzzingMode())
				return;
			auto &f = driver->getConf()->dumpFuzzingValSeqsFile;
			std::ofstream fs(f);

			for (auto &&[tid, trie] : ((GenMCDriver *)driver)->seenValues) {
				trie.dumpDot("G" + std::to_string(tid), fs);
			}
		}

		const GenMCDriver *driver;
		const bool dumpCoverage;
		const bool dumpVal;
		const bool dumpTime;
		const std::clock_t c_start;
	};

	/* Fuzzing: Returns true if the graph is interesting. The fuzzer only mutates interesting
	 * graphs */
	bool isInteresting(const ExecutionGraph &g);
	...
	/* Fuzzing: Picks the next graph for mutation from a corpus of graphs */
	Execution fuzzPickNextGraph();

	/* Moves the back Execution(s) of execStack to the front */
	void rotateExecStack(unsigned n = 1);
	...
	/* Mutations */
	struct RevisitCut {};
	using MutationT = std::variant<RevisitCut>;
	const inline static std::unordered_map<MutationPolicy, MutationT> mutTable = {
		{MutationPolicy::RevisitCut, RevisitCut{}}};

	/* Mutation functions for fuzzing (should be independent of driver internals) */

	std::pair<std::unique_ptr<ExecutionGraph>, std::unique_ptr<Revisit>>
	mutate(const ExecutionGraph &g, const Event &r, const Event &w, const MutationT &mut) const
	{
		auto v = [&](const auto &tag) { return mutate(g, r, w, tag); };
		return std::visit(v, mut);
	}
	std::pair<std::unique_ptr<ExecutionGraph>, std::unique_ptr<Revisit>>
	mutate(const ExecutionGraph &g, const Event &r, const Event &w, const RevisitCut) const;

	/* Prepares alternative rf options given a graph g, acccording to whether corresponding
	 * value sequences seen or not. Prioritize unseen ones but also try infrequent ones when */
	std::vector<
		std::tuple<std::unique_ptr<ExecutionGraph>, ChoiceMap, std::unique_ptr<Revisit>>>
	calcMutationOptions(const ExecutionGraph &g, std::optional<unsigned int> bound);
	...
	/* Fuzz: Picks i-th RF among some possible options. Falls back to random if i is not set */
	Event pickRf(ReadLabel *rLab, std::vector<Event> &stores, bool pickEnd = false);

	/* Est: Picks (and sets) a random CO among some possible options */
	void pickRandomCo(WriteLabel *sLab, std::vector<Event> &cos);

	/* Fuzz: Picks (and sets) i-th CO among some possible options. Falls back to random if i is
	 * not set */
	void pickCo(WriteLabel *sLab, std::vector<Event> &cos, bool pickEnd = false);

	/* Fuzz: decides whether to pick co/rf randomly or not (pick maximally) */
	bool shouldPickCoRfRandomly() const;

	/* Fuzz: (re)sets the lastGEmpty flag */
	void setEmptyGraphFlag();
	...
	/* saves the current graph to pdf. The graph may not be finished */
	void fuzzPreviewCurGraph();
	...
	// ValueBoolT seenValues;
	ValueFreqT seenValues;
	std::deque<Execution> fuzzSeeds;
	...
	/* the hash value of the last graph */
	std::optional<GraphHashT> lastHash{};

	/* Fuzz: is the current graph constructed from an empty one? */
	bool lastGEmpty = false;
	Stamp lastGStamp = 0;
